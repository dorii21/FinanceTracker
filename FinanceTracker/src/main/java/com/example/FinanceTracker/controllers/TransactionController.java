...
public class TransactionController {
    ...
    @GetMapping
    public ResponseEntity<List<TransactionDTO>> filterByCategory(@RequestBody Category category){
        return ResponseEntity.ok(transactionService.filterByCategory(category));
    }

    @GetMapping
    public ResponseEntity<List<TransactionDTO>> filterByAmountBetween(@RequestBody int min, int max){
        return ResponseEntity.ok(transactionService.filterByAmountBetween(min,max));
    }

    @GetMapping
    public ResponseEntity<List<TransactionDTO>> filterByDateBetween(@RequestBody LocalDate min, LocalDate max){
